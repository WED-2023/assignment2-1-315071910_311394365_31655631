<template>
  <div>
    ...
    <div class="favorites-container">
      ...
      <div v-if="recipes.length" class="favorites-grid">
        <RecipePreview v-for="r in recipes" :key="r.id" :recipe="r" />
      </div>
      <!-- If there are no recipes, display a message -->
      <div v-else class="no-favorites">No favorite recipes yet.</div>
    </div>
  </div>
</template>
<script>
import RecipePreview from "../components/RecipePreview"; // Import the RecipePreview component
import { mockGetFavoriteRecipes } from "../services/user"; // Import the mock service to get favorite recipes
...
export default {
  ...
  data() {
    return {
      recipes: [], // Initialize an empty array for storing favorite recipes
    };
  },
  mounted() {
  ...
  methods: {
    // Method to fetch favorite recipes and update the recipes array
    async updateRecipes() {
      try {
        const response = await mockGetFavoriteRecipes(); // Fetch the favorite recipes
        this.recipes = response.data.recipes; // Update the recipes array with the fetched data
      } catch (error) {
        console.error("Error fetching favorite recipes:", error); // Log any errors to the console
      }
    },
  },
};
...
/* Container for the favorite recipes section */
.favorites-container {
  padding: 20px;
  background: #f8f9fa;
  border-radius: 8px;
...
/* Grid styling for the favorite recipes */
.favorites-grid {
  display: grid;
...
/* Message displayed when there are no favorite recipes */
.no-favorites {
  font-size: 1.2em;
  color: #888;
  text-align: center;
